...
export class SearchComponent implements OnInit {
  ...
  constructor(private store:Store<AppState>,private bedsService:BedsService,private router:Router) { }
...
userId:number=0
  ...
  ngOnInit(): void {
   ...
   currentPageSub = this.store.select('properties').subscribe(
...
currentPageSub = this.store.select('hostOruser').subscribe(
(props: number) => {
