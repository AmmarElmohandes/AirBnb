...
export class GuestsComponent implements OnInit {
  ...
  constructor(private router: Router, private guestService: GuestService,private store:Store<AppState>) {this.propertyId=store.select('propertyId') }
  error:string=""
...
this.guest.NoOfAdultGuests=0
    ...
    currentPageSub = this.propertyId.subscribe(
      ...
      this.guestService.guest(this.guest).subscribe(a=>{console.log(a);this.success="successfuly added",this.submitted=false},err=>this.error=err.error)
    }
